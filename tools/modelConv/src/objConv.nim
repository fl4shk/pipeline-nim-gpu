...
const
  ...
  #]
  coplRcidxVidxA2d: array[rcidxLim, array[3, uint]] = [
    [1, 0, 2], # rcidxFront: y changes if not coplanar
    [2, 1, 0], # rcidxLeft: x changes if not coplanar
    [2, 0, 1], # rcidxBottom: z changes if not coplanar
  ]
  coplCidxVidxA2d: array[cidxLim, array[3, uint]] = [
...
proc isAdjInPlane(
  left: var array[quadVertArrLen, Triple[int32]],
  right: var array[quadVertArrLen, Triple[int32]],
): bool =
  
  var myCnt: uint = 0
  let mySet = toHashSet(right)
  var mySeq: seq[uint]
  for idx in 0 ..< quadVertArrLen:
    #for jdx in 0 ..< quadVertArrLen:
    #  #if left.vArr[idx] == right.vArr[jdx]:
    if left[idx] in mySet:
      mySeq.add uint(idx)
  #if mySeq.len() == 2:
  #  myCnt += 1

    #foundSameArr[idx] = toAdd
  #return (myCnt == 2)
  return (mySeq.len() == 2)

proc fastIsAdjCoplanar(
  left: var CubeUnit,
  right: var CubeUnit,
): bool =
  doAssert(left.rcidx.isSome())
  doAssert(right.rcidx.isSome())
  #let chngArr = addr coplRcidxVidxA2d
  ...
  ##var foundSameArr: array[quadVertArrLen, seq[uint]]
  #var myCnt: uint = 0
  #let mySet = toHashSet(right.vArr)
  #var mySeq: seq[uint]
  #for idx in 0 ..< quadVertArrLen:
  #  #for jdx in 0 ..< quadVertArrLen:
  #  #  #if left.vArr[idx] == right.vArr[jdx]:
  #  if left.vArr[idx] in mySet:
  #    mySeq.add uint(idx)
  ##if mySeq.len() == 2:
  ##  myCnt += 1
...
#    return false
#  else:
#    return fastIsAdjCoplanar(left=left, right=right)
#
#  #case left.rcidx.get():
#  #of rcidxFront:
...
proc toCubeDataInfoArr(
  ...
  #for unitIdx in cData.cUnitIdxSeq:
  #  let cUnit: ptr CubeUnit = addr cUnitSeq[unitIdx]
  #  if cUnit.cidx.isSome:
  #    case cUnit.cidx.get():
  #    of cidxFront:
  #      cUnit.cidx = some(cidxBack)
  #    of cidxBack:
  #      cUnit.cidx = some(cidxFront)
  #    of cidxLeft:
  #      cUnit.cidx = some(cidxRight)
  #    of cidxRight:
  #      cUnit.cidx = some(cidxLeft)
  #    of cidxBottom:
  #      cUnit.cidx = some(cidxTop)
  #    of cidxTop:
  #      cUnit.cidx = some(cidxBottom)
  #    else:
  #      doAssert(false)
  #  else:
  #    doAssert(false)
  for k, v in myRciTbl:
    ...
    for item in v:
      ...
      if item.cidx.isSome:
        ...
        #result[revCubeIdxArr[item.cidx.get()] = item
...
proc mkCubeUnit(
  ...
  #  doAssert(false)
  doAssert(foundVn)
  #echo "dbg cUnit: begin"
  #var myFound: bool = false
  #for i in 0 ..< cUnit.vArr.len():
  #  #for j in 0 ..< myOutp[i].len():
  #  #  if myOutp[i][0].v[1] == 32:
  #  #    myFound = true
  #if myFound:
  #  echo "dbg cUnit test: " & $i & ": " & cUnit
  ##echo "dbg cUnit: end"
  result = cUnit
...
proc addOutpFace(
  ...
  foundQuadTbl: var Table[HashSet[Triple[int32]], seq[uint]],
  ...
  let vSet = toHashSet(vArr)
  if vSet notin foundQuadTbl:
    var toAdd: seq[uint]
    toAdd.add idx
    foundQuadTbl[vSet] = toAdd
  else:
    #discard
    echo vArr
    doAssert(
...
proc rectCoplsFirst(
  ...
  proc myFloodFill(
    ...
    while deq.len() > 0:
      ...
      if (
        ...
        continue
      let myAdjSeq0 = addr adjSeq[jdx][cidx]
      if myAdjSeq0[].len() > 0:
        continue
      ...
      if quadVt0 == quadVt1:
        ...
        var tempToSort: array[2, array[quadVertArrLen, Triple[int32]]]
        for myIdx in 0 ..< quadVertArrLen:
          tempToSort[0][myIdx] =  cUnit1.vArr[myIdx]
        let tempSortSeq = tempToSort[0].sorted()
        for myIdx in 0 ..< quadVertArrLen:
          tempToSort[1][myIdx] = tempSortSeq[myIdx]
        myRcmOutpSeq.add tempToSort[1]
        #--------
        ...
        for ckdx in CubeIdx(0) ..< cidxLim:
          ...
          for zdx in 0 ..< myAdjSeq1[].len():
            ...
            #if (
            #  (
            #    adjSeq[kdx][cidx].len() > 0
            #  ) or (
            #    #adjSeq[kdx][ckdx].len() > 0
            #    #adjSeq[kdx][revCubeIdxArr[ckdx]].len() > 0
            #    false
            #  )
            #):
  ...
  for idx in 0 ..< self.cDataOptS2d[^1].len():
    ...
    #echo "testificate:" 
  for idx in 0 ..< self.cDataOptS2d[^1].len():
    let myCidxValidArr = addr tempSeq[idx]
    #var myCidxValidArr: array[cidxLim, bool]
    #for cidx in CubeIdx(0) ..< cidxLim:
    #  #myCidxValidArr[cidx] = true
...
proc rectCoplsSecond(
  ...
  for cidx in CubeIdx(0) ..< cidxLim:
    ...
    #for outerIdx, mainIdx in rCoplIdxTbl:
      ...
      var outerTemp: int32 = 0
      var prevOuterTemp: int32 = 0
      ...
      proc finishStrip(
        ...
        var myOuterLeft: int32 = outerLeft #outerTemp #
        var myOuterRight: int32 = outerRight #outerTemp + 1 #
        ...
        if doPrev:
          ...
          #myOuterLeft = prevOuterTemp
          #myOuterRight = prevOuterTemp + 1
      ...
      #for idx in 0 ..< myInp.len():
      while idx < myInp.len():
        var myElem = addr myInp[idx]
        #var shouldAdd2: bool = false
        ...
        let nextUnconnStrip: bool = (
          ...
          (
            (
              ...
              abs(right - prevRight) > 1
            )
          )
        )
        let nextVertStrip: bool = abs(outerRight - prevOuterRight) >= 1
        if idx > 0:
          echo (
            (
              "unconn, vert: " 
        ...
        #  )
        #):
        #  #let prevMyElem = [addr myInp[idx - 1], addr myInp[idx - 2]]
        #  if (
        #    (
        #      nextUnconnStrip
        #    ) or (
        #      nextVertStrip
        #      #abs(outerLeft - prevOuterLeft) == 1
        #    )
        #  ):
        #    didFinishStrip = 1
        #    echo (
      ...
      if true:
        ...
        ##outerTemp = min(outerLeft, outerRight)
        ##outerTemp = outerLeft
        ...
        #      #) or 
        #      (
        #        abs(outerRight - prevOuterRight) == 1
        #        #outerRight != prevOuterRight
        #        #outerTemp != prevOuterTemp
        #      )
        #      #and (
        #      #  #(uint(cidx) mod 2) == 0
        #      #  #true
        ...
        #    "intermediate finishStrip(): " & $cidx
        #  )
        #  #if abs(left - prevLeft) > 1:
        #  if abs(outerRight - prevOuterRight) != 0:
        #    didFinishStrip = 1
        #    #finishStrip(doPrev=true)
        #  else:
        #    didFinishStrip = 2
        #    #finishStrip(doPrev=false)
      ...
      #  #else:
      #  finishStrip(doPrev=true)
      #if not didFinishStrip and outerRight == prevOuterRight:
      #  echo (
      #    "last finishStrip(): " & $cidx
      #  )
      #  finishStrip(doPrev=true)
